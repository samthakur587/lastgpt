...
nltk.download(
    "punkt"
)  # C:\Users\pranj\OneDrive\djangio\Compfox-Assesment\AI-Features\Search-API\Dockerfile
...
llm = OpenAI(temperature=0.4, model_name="gpt-3.5-turbo-1106")
...
async def upload_file(user_id: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={"error": f"Error occurred during file upload: {str(err)}"},
        )
...
async def askqa(user_id: str, query: str = "brief explanation of this case"):
    try:
        ...
        return JSONResponse(
            status_code=404,
            content={
                "error": f"File not found. for user {user_id} First upload the docs: {str(file_err)}"
            },
        )
...
async def clearall_user(user_id: str):
    try:
        ...
        return JSONResponse(
            status_code=404,
            content={
                "error": f"Cache does not exist for user {user_id}: {str(file_err)}"
            },
        )
...
async def clear_all():
    try:
        ...
        return JSONResponse(
            status_code=404,
            content={"error": f"All caches have already been cleared: {str(file_err)}"},
        )
        ...
        highlighted_phrases.append(
            {
                ...
                "highlighted_phrase": get_highlighted_phrases(
                    cleaned_tokens, highlightable_text["text"]
                ),
            }
        )
